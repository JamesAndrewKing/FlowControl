...
eng.load('ssm_controller_workspace_volume_body_force_new.mat', nargout=0)
...
def run_forced_simulation(Re, save_dir, num_steps, forcing_amplitude, forcing_frequency):
    ...
    params_ic = flowsolverparameters.ParamIC(
        xloc=2.0, yloc=0.0, radius=0.5, amplitude=3.0
    )
    ...
    dof_indices_1 = find_actuator_dof_indices(fs.V, np.array([0.0, 2.5]), 0.6)
    ...
    ###################### Constant gain control (stabilizes) ######################
    ...
    for i in range(fs.params_time.num_steps):
        ...
        if len(u_scalar_history) == buffer_length and t_now>30.0:
            u_ctrl_matlab = eng.ssm_mpc_controller(
...
if __name__ == "__main__":
    ...
    forced_dir = base_dir / f"Re{Re}_volume_body_force_try_mpc" / "run1"
