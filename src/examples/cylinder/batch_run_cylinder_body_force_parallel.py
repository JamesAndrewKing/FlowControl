...
from examples.cylinder.batch_run_cylinder_body_force import run_forced_simulation
...
def run_single_simulation(args):
    ...
    forcing_amplitude, forcing_frequency, Re, save_dir, num_steps = args

    print(f"Starting simulation freq={forcing_frequency:.3f}, amp={forcing_amplitude:.3f} in process {mp.current_process().name}")

    try:
        run_forced_simulation(Re, save_dir, num_steps, forcing_amplitude, forcing_frequency)
...
if __name__ == "__main__":
    ...
    forcing_frequencies = [0.2, 0.5, 0.7, 1.0] # In rad/s
    # forcing_amplitudes = [0.2, 0.5, 0.8]
    forcing_amplitudes = [0.1, 0.2, 0.3, 0.4]
    ...
    for f_amp in forcing_amplitudes:
        for freq in forcing_frequencies:
            ...
            simulation_args.append((f_amp, freq, Re, save_dir, num_steps))
            count += 1
